...
def get_dataset():
    ...
    def get_num_steps(percent = 0.90):
        lens = [
    ...
    # test_feature_1 = test_feature_1[:,::-1,:]
    # test_feature_2 = test_feature_2[:,::-1,:]
...
num_hiddens, num_layers, dropout =  50, 2, 0.1
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, encoder, decoder, **kwargs):
        ...
        self.dense = nn.Linear(num_hiddens*2,4)

    def forward(self, enc_X, dec_X, *args):
        enc_outputs = self.encoder(enc_X, *args)
        dec_state = self.decoder.init_state(enc_outputs, *args)
        dec_outputs = self.decoder(dec_X, dec_state)
        enc = enc_outputs[1][0][-1]
        dec = dec_outputs[1][1][0][-1]
        world_cat = torch.concatenate((enc,dec),dim=1)
        out = self.dense(world_cat)
        # import ipdb;ipdb.set_trace()
        return out

num_steps = X_train.shape[1]
lr, num_epochs, device = 0.1, 250, d2l.try_gpu()
...
for epoch in range(num_epochs):
    ...
    for batch in train_iter:
        ...
        # import ipdb;ipdb.set_trace()
        with torch.no_grad():
            _, max= torch.max(output,dim=1)
            metric.add(l.item() * X.size(0),(max == y).sum().item(),y.numel())

    with torch.no_grad():
        net.eval()
        correct = 0
        total = 0
        X_val_tensor, len_val_tensor, y_val_tensor = [x.to(device) for x in val_arrays]
        X1 = get_arg1(X_val_tensor)
        X2 = get_arg2(X_val_tensor)
        len1 = len_val_tensor[:,0]
        len2 = len_val_tensor[:,1]
        outputs = net(X1,X2,len1)
